...
energy_full_jit = jit(total_energy_fn, static_argnames=["coulomb_handler"])


# === Report energy ===
nlist_init = neighbor_fn.allocate(positions)
nlist_init = neighbor_fn.allocate(positions)
E_bond_init, E_angle_init, E_torsion_init, E_improper_init, E_nb_init, e_real_init, e_recip_init, e_self_init, E_coul_init, E_total_init = energy_breakdown_full_jit(positions, 
                                                                                                                                                                     nlist_init, coulomb_handler=coulomb_handler)
E_tot=energy_full_jit(positions, nlist_init, coulomb_handler=coulomb_handler)
...
print(f"Total potential : {E_tot:.6f} kcal/mol")
